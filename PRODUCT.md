---
name: WITHIN
one_liner: An Indonesian electrolyte sachet that puts back exactly what heat and humidity sweat out of you.
stage: pre-launch (coming-soon site); primary goal is waitlist signups
---

# WITHIN — Product Truth

> Product context captured for design work. Product facts only, no visual decisions (those live in DESIGN.md).
> Substituted for a live init interview: product truth was established over this session and the user asked to build; direction was locked with the user via brainstorming (Transparency Lab / Clinical White + Electric).

## What it is
A single-serve electrolyte drink sachet. One sachet mixes into ~500ml of water and is taken during or after training.

## The mechanism (unique claim)
Per sachet: **1000mg sodium, 250mg potassium, 50mg magnesium** — dosed to what a hard hour of sweat actually carries out, not a pinch for taste. Sodium holds the fluid in, potassium moves it into cells, magnesium steadies muscle and nerve.

## Why it exists
Training in Indonesian heat and humidity, sweat cannot evaporate fast enough to cool the body, so the body makes more of it. Every drop carries minerals out, mostly sodium and some potassium. Water alone tops the fluid back up but dilutes what is left. WITHIN replaces both the fluid and the minerals.

## Formula / transparency
Under 8 ingredients. No added sugar, no artificial colors, no artificial flavors, no artificial sweeteners. Mineral salts for the actives, real fruit for flavor, stevia leaf for sweetness. Transparency is a core value: everything on the label earns its place.

## Audience
Indonesian athletes and active people (running, cycling, gym, outdoor work) plus health-conscious buyers researching a supplement before purchase. They want to see clearly what is inside and why the dose is what it is.

## Comparison frame
Positioned against plain water (replaces fluid, no minerals) and mainstream sports drinks (under-dosed minerals, buried under sugar). WITHIN's edge is honest, high electrolyte dosing with nothing artificial.

## Go-to-market
Launching soon. Online first, direct-to-consumer, shipping across Indonesia. Waitlist offers a launch 10% discount. Only live channel pre-launch is Instagram @within__id.

## Voice
Fact-forward, honest, no hype, no filler verbs. Let the numbers and the transparency do the persuading.

## Hard constraints (do not violate)
- **No certification claims.** BPOM and Halal are both *pending, not yet certified*. Never state or imply certified/approved status anywhere on the site.
- **Preserve the waitlist function.** The SignupForm captures email (+ optional Instagram) to Shopify; do not rename its fields or change its submit behavior. Honeypot anti-spam stays.
- Keep FAQ content, structured data (Organization/WebSite/FAQPage JSON-LD), and SEO metadata intact through the redesign.
